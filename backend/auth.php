...
function handleCustomerRegister(array $input) {
    ...
    $token = createJwt(['user_id' => $userId, 'email' => $email, 'name' => $name]);
...
// Customer Login (supports regular customers AND store admin earthenbeauty@gmail.com)
function handleCustomerLogin(array $input) {
    ...
    // 1. Check if this email belongs to an Administrator in the admins table
    $adminStmt = $db->prepare("SELECT id, name, email, password_hash, role FROM admins WHERE email = ? LIMIT 1");
    $adminStmt->execute([$email]);
    $admin = $adminStmt->fetch();

    $isAdminMatch = false;
    if ($admin && verifyUserPassword($password, $admin['password_hash'])) {
        $isAdminMatch = true;
    } else if ($email === 'earthenbeauty@gmail.com' && $password === 'EARTHENBEAUTY') {
        $isAdminMatch = true;
        if (!$admin) {
    ...
    if ($isAdminMatch && $admin) {
        // Issue both Customer JWT and Admin JWT
        $adminToken = createJwt([
            'admin_id' => (int)$admin['id'],
            'email'    => $admin['email'],
            'role'     => $admin['role'] ?? 'super_admin'
        ]);
        $customerToken = createJwt([
        ...
        sendJson([
            ...
            'user'        => [
                ...
                'name'     => $admin['name'],
                'email'    => $admin['email'],
                'phone'    => '+91 82968 91802',
                'is_admin' => true
            ]
        ]);
    }

    // 2. Otherwise, check standard registered customers in the users table
    ...
    if (!$user || !verifyUserPassword($password, $user['password_hash'])) {
        sendJson(['success' => false, 'detail' => 'Invalid email or password. Please try again.'], 401);
    }
...
function handleAdminLogin(array $input) {
    ...
    if (!$admin && $email === 'earthenbeauty@gmail.com' && $password === 'EARTHENBEAUTY') {
        $hash = hashUserPassword('EARTHENBEAUTY');
        $ins = $db->prepare("INSERT INTO admins (name, email, password_hash, role) VALUES (?, ?, ?, ?)");
        $ins->execute(['Nupur (Store Owner)', 'earthenbeauty@gmail.com', $hash, 'super_admin']);
    ...
    } else if (!$admin || !verifyUserPassword($password, $admin['password_hash'])) {
        // Double check default admin fallback
        if ($email === 'earthenbeauty@gmail.com' && $password === 'EARTHENBEAUTY') {
            // Valid default admin
        } else {
