...
(function() {
  ...
  // Seamless Bridge: If user signed in as admin on storefront, bridge session to admin portal
  if (!adminToken) {
    var custUser = null;
    try { custUser = JSON.parse(localStorage.getItem("eb_customer_user") || "null"); } catch (e) {}
    if (custUser && (custUser.is_admin || (custUser.email && custUser.email.toLowerCase() === "earthenbeauty@gmail.com"))) {
      adminToken = localStorage.getItem("eb_customer_token") || null;
      currentAdmin = custUser;
      if (adminToken) {
        localStorage.setItem("eb_admin_token", adminToken);
        localStorage.setItem("eb_admin_user", JSON.stringify(custUser));
      }
    }
  }
  ...
  window.closeProductModal = function() {
    ...
    if (window.AdminMotions && window.AdminMotions.animateModalClose) {
      try {
        window.AdminMotions.animateModalClose("product-modal");
      } catch (e) {}
    }
  };
