...
function initCustomerAuthUI() {
  ...
  if (nav) {
    ...
    var navAdmin = nav.querySelector("#nav-admin-link, .nav-admin-capsule-item");
...
function isUserAdmin(user) {
  ...
  if (user.email && user.email.toLowerCase() === "earthenbeauty@gmail.com") return true;
  if (user.name && user.name.toLowerCase().indexOf("nupur") !== -1) return true;
  var adminToken = localStorage.getItem("eb_admin_token");
  if (adminToken && adminToken.length > 10) return true;
  return false;
}
...
function updateProfileNavDisplay() {
  ...
  if (token && user && user.name) {
    ...
    // Auto-sync admin token if missing
    if (isAdmin && !adminToken) {
      localStorage.setItem("eb_admin_token", token);
      localStorage.setItem("eb_admin_user", JSON.stringify(user));
    }

    // 1. Prominent "👑 Open Studio Admin" link inside profile dropdown
    var existingAdminLink = document.getElementById("dropdown-admin-portal-link");
    if (isAdmin) {
      if (menu && !existingAdminLink) {
        var adminLink = document.createElement("a");
        adminLink.href = "admin.html";
        adminLink.id = "dropdown-admin-portal-link";
        adminLink.className = "dropdown-link dropdown-admin-portal-link";
        adminLink.innerHTML =
          '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#e5b869" stroke-width="2"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>' +
          '<span style="font-weight:700; color:#f8dfa5;">👑 Open Studio Admin</span>' +
          '<span style="margin-left:auto; font-size:0.68rem; background:rgba(229,184,105,0.3); color:#f8dfa5; padding:2px 7px; border-radius:4px; font-weight:700;">PORTAL</span>';

        var headerEl = menu.querySelector(".profile-dropdown-header");
        if (headerEl && headerEl.nextSibling) {
          menu.insertBefore(adminLink, headerEl.nextSibling);
        } else if (menu) {
    ...
    // 2. Glowing "👑 Open Studio" button in header next to profile
    var headerActions = document.querySelector(".header-actions");
    var existingHeaderBtn = document.getElementById("header-admin-studio-btn");
    if (isAdmin) {
      if (headerActions && !existingHeaderBtn) {
        var studioBtn = document.createElement("a");
        studioBtn.href = "admin.html";
        studioBtn.id = "header-admin-studio-btn";
        studioBtn.className = "header-admin-studio-btn";
        studioBtn.innerHTML = "<span>👑</span> <span>Open Studio</span>";
        studioBtn.title = "Open Studio Administration Portal";

        var profileBtn = document.getElementById("header-profile-btn");
        if (profileBtn) {
          headerActions.insertBefore(studioBtn, profileBtn);
        } else {
    ...
    // 3. Ensure Center Nav Capsule stays clean (Home | Shop | Customize | About | Contact only)
    var existingNavAdmin = document.getElementById("nav-admin-link");
    if (existingNavAdmin) existingNavAdmin.remove();
    var navLinks = document.getElementById("nav-links");
    if (navLinks) {
      var extraAdminLi = navLinks.querySelector(".nav-admin-capsule-item");
      if (extraAdminLi) extraAdminLi.remove();
    }

    // 4. Update Profile Button Tooltip
    var pBtn = document.getElementById("header-profile-btn");
    if (pBtn) {
      pBtn.title = isAdmin ? "Signed in as Admin (" + (user.name || "Nupur") + ")" : "Account (" + (user.name || "Customer") + ")";
    }
    ...
    var oldHeaderBtn = document.getElementById("header-admin-studio-btn");
    if (oldHeaderBtn) oldHeaderBtn.remove();
    var oldNavAdmin = document.getElementById("nav-admin-link");
    if (oldNavAdmin) oldNavAdmin.remove();
    var navLinks = document.getElementById("nav-links");
    if (navLinks) {
      var extraAdminLi = navLinks.querySelector(".nav-admin-capsule-item");
      if (extraAdminLi) extraAdminLi.remove();
  ...
  div.innerHTML =
    ...
    '<div class="customer-modal-overlay" id="order-success-modal">' +
      '<div class="customer-modal-card order-success-card">' +
        ...
        '</div>' +
        '<h3 style="font-family: var(--font-heading); font-size: 1.8rem; font-weight: 500; color: #ffffff;">Order Confirmed!</h3>' +
...
async function submitCustomerSignIn(e) {
  ...
  try {
    ...
    if (data.admin_token) {
      localStorage.setItem("eb_admin_token", data.admin_token);
      localStorage.setItem("eb_admin_user", JSON.stringify(data.user));
    }
    ...
    if (data.is_admin) {
      // Direct notification that admin mode is active
      var studioBtn = document.getElementById("header-admin-studio-btn");
      if (studioBtn) {
        studioBtn.style.animation = "pulse 1s 2";
      }
    }
